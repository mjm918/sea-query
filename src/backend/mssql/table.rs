...
impl TableBuilder for MssqlQueryBuilder {
	...
	fn prepare_column_type(&self, column_type: &ColumnType, sql: &mut dyn SqlWriter) {
		write!(
			...
			match column_type {
				...
				ColumnType::Text => "VarChar(MAX)".into(),
				...
				ColumnType::Decimal(precision) => "real".into(),
				...
				ColumnType::Binary(blob_size) => "VARBINARY(MAX)".into(),
				ColumnType::VarBinary(length) => format!("varbinary({length})"),
				ColumnType::Bit(length) => "unsupported".into(),
				ColumnType::VarBit(length) => "unsupported".into(),
				ColumnType::Boolean => "bool".into(),
				ColumnType::Money(precision) => "unsupported".into(),
				ColumnType::Json => "unsupported".into(),
				ColumnType::JsonBinary => "unsupported".into(),
				ColumnType::Uuid => "varchar(16)".into(),
				ColumnType::Custom(iden) => iden.to_string(),
				ColumnType::Enum { variants, .. } => "unsupported".into(),
				ColumnType::Array(_) => unimplemented!("Array is not available in MSSQL."),
				ColumnType::Cidr => unimplemented!("Cidr is not available in MSSQL."),
				ColumnType::Inet => unimplemented!("Inet is not available in MSSQL."),
				ColumnType::MacAddr => unimplemented!("MacAddr is not available in MSSQL."),
			}
		)
			.unwrap();
		if matches!(
